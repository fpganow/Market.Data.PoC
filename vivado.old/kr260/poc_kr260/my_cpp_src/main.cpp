...
//	int status = 0;
	...
	while (keepLooping)
	{
		EthFrameNode* packet = dmaReader.read_packets();
		if (packet == nullptr)
		{
			printf("No new data\n");
		}
...
//    	u32 buf_to_recv_len = 1024;
    	EthFrameNode* frames = dmaReader.read_packets();
    	if (frames == nullptr)
    	{
    	...
    	{
    		EthFrameNode* current = frames;
    		EthFrameNode* prev;
    		while (current != nullptr) {
    			std::cout << "Frame Length: " << current->packet_length << "\n";
    			for (int i=0; i < current->packet_length; i++) {
    				printf("  0x%016lx\n", current->packet_data[i]);
    			}
    			prev = current;
